// src/app/pages/leadership/manage-jobs.component.ts
/*
    ...
    <div class="space-y-6">
      <div class="p-6 bg-white rounded-lg shadow">
        ...
        <form [formGroup]="jobForm" (ngSubmit)="createJob()" class="mt-4 space-y-4">
          ...
          <input formControlName="country" (keyup)="getVendorsForCountry()" placeholder="Country (e.g., USA)" class="w-full px-3 py-2 border rounded">
...
export class ManageJobsComponent implements OnInit {
  ...
  createJob() {
    ...
    this.leadershipService.createJob(this.jobForm.value).subscribe(() => {
      this.loadJobs();
      this.jobForm.reset({ country: 'USA', assignedVendorPublicIds: [] });
...
export class ManageJobsComponent implements OnInit {
  ...
  jobForm: FormGroup;
  countries = COUNTRIES;
  countryControl = new FormControl('USA');
  ...
  createJob() {
    ...
    this.leadershipService.createJob(this.jobForm.value).subscribe(() => {
      this.loadJobs();
      this.jobForm.reset({ country: 'USA', assignedVendorPublicIds: [] });
      this.countryControl.setValue('USA');
      this.getVendorsForCountry();
    });
  }
}
